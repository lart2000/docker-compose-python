# apps/proxies/tests/test_models.py
# Python imports
...
class ProxyTestCase(TestCase):
    def setUp(self):
        ...
        self.field_list = [field.name for field in Proxy._meta.get_fields()]
    ...
    def test_have_country_relation(self):
        self.assertTrue(self.proxy.country.__class__ is Country)

    def test_have_user_relation(self):
        self.assertTrue(self.proxy.user.__class__ is get_user_model())

    def test_method_str(self):
        self.assertIn(self.proxy.user.username, self.proxy.__str__())
